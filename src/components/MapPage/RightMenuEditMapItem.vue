<template>
  <v-navigation-drawer class="RightMenuEditMapItem" v-model="value_" :width="width" right dark disable-resize-watcher
    fixed hide-overlay app clipped>
    <div class="RightMenuEditMapItem__inner">
      ...
      <div class="RightMenuEditMapItem__section">
        <!-- Название -->
        <v-text-field v-model="copyItem.discipline" label="Введите название" hide-details="auto" :rules="disciplineRules"
          ref="discipline" dense filled dark />
      </div>
        ...
        <v-expansion-panel class="RightMenuEditMapItem__expansion">
          ...
          <v-expansion-panel-content>
            <div class="RightMenuEditMapItem__type-wrapper">
              ...
              <div class="RightMenuEditMapItem__type-row" v-for="(type, i) in copyItem.type.value" :key="i">
                <v-text-field :value="type.amount" :label="getControlTypesLabel(type.control_type_id)" type="number"
                  ref="zet" :rules="hoursRules" hide-details dense filled dark @input="onInputHours(i, $event)" />

                <v-text-field :value="type.zet" :label="getControlTypesLabel(type.control_type_id)" :min="1" :max="10"
                  type="number" ref="zet" :rules="zetRules" hide-details dense filled dark
                  @input="onInputZet(i, $event)" />

                <v-switch :value="type.id_edizm === 2" class="RightMenuEditMapItem__type-row__switch"
                  label="Измерять в неделях" @change="onUpdateUnitsOfMeasurement(i)" />
              </div>

              <v-divider dark class="RightMenuEditMapItem__divider" />

              <div class="RightMenuEditMapItem__type-row">
                <v-text-field :value="sumHours" label="Сумма часов" type="number" hide-details readonly dense filled
                  dark />

                <v-text-field :value="sumZet" label="Сумма ЗЕТ" type="number" hide-details readonly dense filled dark />
              </div>

              <v-divider dark class="RightMenuEditMapItem__divider" />

              <v-select :value="selectedControlTypes" :items="allValueTypes" label="Нагрузки" item-text="control"
                item-disabled="disabled" return-object filled dense hide-details multiple
                no-data-text="Доступные нагрузки отсутствуют" @change="onSelectControlTypes">
        ...
        <v-expansion-panel class="RightMenuEditMapItem__expansion">
          ...
          <v-expansion-panel-content>
            <v-radio-group v-model="controlTypes">
              <v-radio v-for="control in allControlTypes" :key="control.id" :label="`${control.name}`"
                :value="control.id">
                {{ control.name }}
              </v-radio>
            </v-radio-group>
      ...
      <div class="RightMenuEditMapItem__actions">
        ...
        <v-btn class="RightMenuEditMapItem__save-btn" color="success" :loading="isLoading" @click="onSave">
          <span>Сохранить</span>
...
export default {
  ...
  methods: {
    ...
    onInputHours(index, value) {
      ...
      this.copyItem.type.value[index].zet = this.calculatedZet(+value, this.copyItem.type.value[index].id_edizm)
    ...
    onInputZet(index, value) {
      ...
      if (this.copyItem.type.value[index].id_edizm === 2) {
        this.copyItem.type.value[index].amount = +value / this.MapsService.WEEKQUEALSZET
      }
      else {
        this.copyItem.type.value[index].amount = +value * this.MapsService.ZETQUEALSHOURS
      }
      ...
      return this.copyItem.type.value.reduce(
        (accumulator, currentValue) => {
          if (currentValue.id_edizm === 1 || field === 'zet') {
            ...
            return accumulator + (currentValue[field] * this.MapsService.WEEKQUEALSHOURS)
          }
        }, 0)
    },
    ...
    onSelectControlTypes(e) {
      if (e.length < this.copyItem.type.value.length) {
        this.copyItem.type.value = this.copyItem.type.value.filter(el => e.find(_el => _el.id === el.control_type_id))
    ...
    onUpdateUnitsOfMeasurement(index) {
      this.copyItem.type.value[index].id_edizm = this.copyItem.type.value[index].id_edizm % 2 + 1

      this.copyItem.type.value[index].amount = this.copyItem.type.value[index].id_edizm === 2
        ? this.copyItem.type.value[index].zet / this.MapsService.WEEKQUEALSZET
        : this.copyItem.type.value[index].zet * this.MapsService.ZETQUEALSHOURS
    },
