...
export default {
  ...
  computed: {
    stats() {
      const typeValueByStats = MapsService.controlTypes.value.filter(item => item.name === 'СРС' || item.name === 'Лекции')
      const stats = {
        value: {},
        session: {},
      }
        ...
        item.type.value.forEach((type) => {
          ...
          if (typeValue) {
            if (stats.value[type.control_type_id]) {
              if (type.id_edizm === 2) {
                stats.value[type.control_type_id] += type.amount * MapsService.WEEKQUEALSHOURS
              }
              else {
